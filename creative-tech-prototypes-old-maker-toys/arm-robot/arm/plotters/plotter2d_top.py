...
class ArmPlotterTopView:
    def __init__(self):
        pass

    @staticmethod
    def update(fig, arm):
        ArmPlotterTopView._draw_arm(fig, arm)
        ArmPlotterTopView._draw_scene(fig, arm)
    ...
    def _draw_arm(fig, arm):
        ...
        for i in range(0, 3):
            ArmPlotterTopView._draw_line(fig, i, p[i], p[i + 1])

        for i, point in zip(range(3, 7), p):
            ArmPlotterTopView._draw_point(fig, i, point)
