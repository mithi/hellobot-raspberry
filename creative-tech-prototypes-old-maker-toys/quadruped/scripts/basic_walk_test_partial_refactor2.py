...
class QuadrupedCore:
  ...
  def bend_up(self, position, delay=0):
    knees[position].pose(KNEE_BENTUP)
    ankles[position].pose(ANKLE_BENTUP)
    sleep(delay)
  ...
  def pose(self, position, hip_angle, knee_angle, ankle_angle, delay):
    hips[position].pose(hip_angle)
    knees[position].pose(knee_angle)
    ankles[position].pose(ankle_angle)
    sleep(delay)
...
hip_front_left = Joint(0, 140, 460, ANGLE_MIN, ANGLE_MAX)
